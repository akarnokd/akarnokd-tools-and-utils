...
public class CryptoUtilsTest {
	...
	public void testPasswordEncryptDecrypt() {
		...
		byte[] salt = CryptoUtils.generateSalt(8);

		byte[] data = "secret".getBytes();
		
		Pair<byte[], byte[]> edata = CryptoUtils.passwordEncrypt(password, salt, data);
		
		byte[] ddata = CryptoUtils.passwordDecrypt(password, salt, edata.first, edata.second);
